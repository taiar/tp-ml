...
int main(int argc, char ** argv){
  ...
  if(aux != trainCnt) {
    fprintf(stderr, "ERROR: Train set size missmatch! %i images but %i labels.", aux, trainCnt);
    exit(-1);
  }
  ...
  // LENET-based params
  CCNN * Net = new CCNN(convCnt, hiddenCnt, featureMapCnt, kernelSizes, stepSizes, hiddenUnits,
    classCnt, imageR);
  ...
  printf("Testing has commenced.\n");
  {
    ...
    for (size_t i = 0; i < classCnt; i++)
      printf("%i  %i/%i (%.3lf%% accuracy)\n", i, errorByClass[i], countByClass[i],
        100.0 - 100.0*errorByClass[i] / countByClass[i]);
    free(countByClass);
    free(errorByClass);
  }
